...
/**
 * This class defines the structure of the 'event_category' table.
 *
 *
 *
 ...
 *
 */
class EventCategoryTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.EventCategoryTableMap';
     ...
     */
    const OM_CLASS = '\\EventCategory';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'EventCategory';
    ...
    {
        // attributes
        $this->setName('event_category');
        $this->setPhpName('EventCategory');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\EventCategory');
        $this->setPackage('');
  ...
  array (
     ...
     * and findPk*() calls.
     *
     * @param \EventCategory $obj A \EventCategory object.
     ...
     *
     * @param mixed $value A \EventCategory object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \EventCategory) {
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \EventCategory object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    {
        return $withPrefix ? EventCategoryTableMap::CLASS_DEFAULT : EventCategoryTableMap::OM_CLASS;
    }
     ...
     * @return array           (EventCategory object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = EventCategoryTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = EventCategoryTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + EventCategoryTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = EventCategoryTableMap::OM_CLASS;
            /** @var EventCategory $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            EventCategoryTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = EventCategoryTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = EventCategoryTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(EventCategoryTableMap::DATABASE_NAME)->getTable(EventCategoryTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(EventCategoryTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(EventCategoryTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new EventCategoryTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a EventCategory or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or EventCategory object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \EventCategory) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(EventCategoryTableMap::DATABASE_NAME);
        ...
        $query = EventCategoryQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            EventCategoryTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                EventCategoryTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return EventCategoryQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a EventCategory or Criteria object.
     *
     * @param mixed               $criteria Criteria or EventCategory object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from EventCategory object
        }
...
} // EventCategoryTableMap
// This is the static code needed to register the TableMap for this table with the main Propel class.
//
EventCategoryTableMap::buildTableMap();
