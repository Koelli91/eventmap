...
abstract class Category implements ActiveRecordInterface 
{
    ...
    public function setEventCategories(Collection $eventCategories, ConnectionInterface $con = null)
    {
        ...
        foreach ($eventCategoriesToDelete as $eventCategoryRemoved) {
            $eventCategoryRemoved->setCategory(null);
        }
    ...
    protected function doAddEventCategory(ChildEventCategory $eventCategory)
    {
        ...
        $eventCategory->setCategory($this);
    }
     ...
     * @return ObjectCollection|ChildEventCategory[] List of ChildEventCategory objects
     */
    public function getEventCategoriesJoinEvent(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildEventCategoryQuery::create(null, $criteria);
    ...
    protected function doAddEvent(ChildEvent $event)
    {
        ...
        if (!$event->isCategoriesLoaded()) {
            $event->initCategories();
            $event->getCategories()->push($this);
        } elseif (!$event->getCategories()->contains($this)) {
            $event->getCategories()->push($this);
        }
    ...
    {
        if ($this->getEvents()->contains($event)) { $eventCategory = new ChildEventCategory();

            $eventCategory->setEvent($event);
            if ($event->isCategoriesLoaded()) {
                //remove the back reference if available
                $event->getCategories()->removeObject($this);
            }
